...
class _TaskListItemState extends State<TaskListItem> {
        ...
        children: [
          CustomSlidableAction(
            onPressed: (context) async {
              await _toggleTaskPriority(taskProvider);
            },
          ...
          CustomSlidableAction(
            onPressed: (context) async {
              await _delayTaskToday(taskProvider);
            },
          ...
          CustomSlidableAction(
            onPressed: (context) async {
              await _delayTaskTomorrow(taskProvider);
            },
          ...
          CustomSlidableAction(
            onPressed: (context) async {
              await _delayTaskNextWeek(taskProvider);
            },
          ...
          CustomSlidableAction(
            onPressed: (context) async {
              await _delayTaskNextMonth(taskProvider);
            },
  ...
  // 将任务延期到今天
  Future<void> _delayTaskToday(TaskProvider taskProvider) async {
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
  ...
  // 将任务延期到明天
  Future<void> _delayTaskTomorrow(TaskProvider taskProvider) async {
    final now = DateTime.now();
    final tomorrow = DateTime(now.year, now.month, now.day + 1);
  ...
  // 将任务延期到下周
  Future<void> _delayTaskNextWeek(TaskProvider taskProvider) async {
    final now = DateTime.now();
  ...
  // 将任务延期到下月
  Future<void> _delayTaskNextMonth(TaskProvider taskProvider) async {
    final now = DateTime.now();
  ...
  // 切换任务优先级
  Future<void> _toggleTaskPriority(TaskProvider taskProvider) async {
    // 在高优先级和无优先级之间切换
